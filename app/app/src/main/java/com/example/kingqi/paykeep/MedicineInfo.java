...
public class MedicineInfo extends AppCompatActivity {
    ...
    private int mod = 0;
    private String ip = "121.36.216.202";
    @Override
...
//        toolbar.setSubtitle(m+"月账单:");
        ...
        fab.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(final View view) {
                //进入编辑页面：
                Intent intent = new Intent(MedicineInfo.this,EditPay.class);
                ...
                try {
                    ...
                    if (rd.isEmpty()){
                        runOnUiThread(new Runnable() {
                            ...
                            public void run() {
                                if (n==1)
                                    Toast.makeText(MedicineInfo.this,"啊咧咧~下次再上传~",Toast.LENGTH_SHORT).show();
                            }
                        });
                    }else {
                        if (rd.equals("add_ok")){
...
//                                    refreshSum();
                                    if (n==1)
                                        Toast.makeText(MedicineInfo.this,"添加成功",Toast.LENGTH_SHORT).show();
                                }
                            });
                        }else {
                            runOnUiThread(new Runnable() {
                ...
                try {
                    ...
                    if (rd.isEmpty()){
                        runOnUiThread(new Runnable() {
                            @Override
                            public void run() {
                                Toast.makeText(MedicineInfo.this,"啊咧咧~电波无法到达~",Toast.LENGTH_SHORT).show();
                            }
                        });
                    }else {
...
//                                    refreshSum();
                                    Toast.makeText(MedicineInfo.this,"消除成功",Toast.LENGTH_SHORT).show();
                                }
                            });
                        }else {
                            runOnUiThread(new Runnable() {
                                @Override
                                public void run() {
                                    Toast.makeText(MedicineInfo.this,"消除失败",Toast.LENGTH_SHORT).show();
                                }
                            });
                        }
                    }
